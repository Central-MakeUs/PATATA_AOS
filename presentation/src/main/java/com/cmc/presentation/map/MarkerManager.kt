package com.cmc.presentation.map

import android.util.Log
import com.cmc.domain.model.SpotCategory
import com.cmc.presentation.model.SpotCategoryItem
import com.cmc.presentation.model.SpotUiModel
import com.naver.maps.geometry.LatLng
import com.naver.maps.map.NaverMap
import com.naver.maps.map.overlay.Marker
import com.naver.maps.map.overlay.OverlayImage

class MarkerManager(private val naverMap: NaverMap) {
    private val markerMap: MutableMap<SpotUiModel, Marker> = mutableMapOf()

    fun updateMarkersWithData(newDataList: List<SpotUiModel>?) {
        if (newDataList.isNullOrEmpty()) return
        Log.d("testLog", "updateMarkersWithData")

        val dataSet = markerMap.keys
        val newDataSet = newDataList.toSet()

        // 제거할 데이터 처리
        dataSet.subtract(newDataSet).forEach { removeData ->
            markerMap[removeData]?.map = null
            markerMap.remove(removeData)
        }

        // 추가할 데이터 처리
        newDataSet.subtract(dataSet).forEach { data ->
            markerMap[data] = Marker().apply {
                position = LatLng(data.latitude, data.longitude)
                icon = OverlayImage.fromResource(
                    SpotCategoryItem(SpotCategory.fromId(data.categoryId))
                        .getMarkerIcon()
                )
                map = naverMap
            }
        }
    }
}
