...
class TokenPreferences @Inject constructor(
    ...
    val accessTokenFlow: Flow<String?> = context.tokenPreferences.data
        .map { it[KEY_ACCESS_TOKEN] }

    val googleAccessTokenFlow: Flow<String?> = context.tokenPreferences.data
        .map { it[KEY_GOOGLE_ACCESS_TOKEN] }
    ...
    suspend fun getRefreshToken(forceRefresh: Boolean = false): String? {
        return context.tokenPreferences.data
            .map { preferences -> preferences[KEY_REFRESH_TOKEN] }
            .first()
    }

    suspend fun getGoogleAccessToken(forceRefresh: Boolean = false): String? {
        return context.tokenPreferences.data
